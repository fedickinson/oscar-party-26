...
/** Synchronous byte SHA-256 for pure browser/Node validation paths. */
export function sha256BytesHex(bytes: Uint8Array): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const priorTwo = words[index - 2]
      const sigmaZero = rotateRight(prior, 7) ^ rotateRight(prior, 18) ^ (prior >>> 3)
      const sigmaOne = rotateRight(priorTwo, 17) ^ rotateRight(priorTwo, 19) ^ (priorTwo >>> 10)
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const first = (h + upperOne + choice + ROUND_CONSTANTS[index] + words[index]) >>> 0
...
/** Synchronous UTF-8 SHA-256 for pure browser/Node validation paths. */
