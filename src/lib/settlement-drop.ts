...
function renderQuoteDesk(
  ...
  const controls = quotes.length > 1
    ? `<div class="quote-controls" aria-label="Pundit desk controls">
          <button type="button" data-quote-prev aria-label="Previous take" disabled>Previous</button>
...
(function(){
  ...
  document.addEventListener('click',function(event){
    ...
    if(desk&&(event.target.closest('[data-quote-next]')||event.target.closest('[data-quote-prev]'))){
      ...
      var wanted=active+(event.target.closest('[data-quote-next]')?1:-1);
      if(wanted<0||wanted>=quotes.length)return;
      quotes[active].classList.remove('active');
      quotes[wanted].classList.add('active');desk.querySelector('[data-quote-position]').textContent=String(wanted+1);
      var quotePrev=desk.querySelector('[data-quote-prev]');var quoteNext=desk.querySelector('[data-quote-next]');
      var quoteFocus=document.activeElement;
      quotePrev.disabled=wanted===0;quoteNext.disabled=wanted===quotes.length-1;
      if(quoteFocus===quotePrev&&quotePrev.disabled)quoteNext.focus();
      if(quoteFocus===quoteNext&&quoteNext.disabled)quotePrev.focus();
      return;
    }
