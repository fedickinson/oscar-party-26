 ...
 *   1. UPSERT room_winners (room_id, category_id, winner_id)
 ...
 *      WHERE category_id = categoryId AND nominee_id = nomineeId AND room_id = roomId
...
export function useAdmin(roomId: string | undefined): AdminState {
    ...
    async function load() {
      ...
      if (catRes.data) {
        ...
        const hydrated: CategoryWithNominees[] = (catRes.data as any[]).map((cat) => ({
          ...cat,
          winner_id: winnerMap.get(cat.id) ?? null,
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          ...
          setCategories((prev) =>
            prev.map((c) => (c.id === rw.category_id ? { ...c, winner_id: null } : c)),
          )
        },
      )
      .subscribe()
  ...
  return { categories, winnerSetAt, isLoading, setWinner, undoWinner }
}
