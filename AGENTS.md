...
| Draft command dogfood | `npx tsx scripts/dogfood-draft-command.mts` | Focused local concurrency and compatibility proof; binds a retained Results Night draft catalog and writes only one disposable room. |
| Scheduled winner dogfood | `npx tsx scripts/dogfood-scheduled-winner-command.mts` | Focused local winner/tie/undo, confidence projection, concurrency and mixed-version proof; binds the retained Results Night proof catalog. |
| Scheduled spotlight dogfood | `npx tsx scripts/dogfood-scheduled-spotlight-command.mts` | Focused local host/revision/open/close concurrency and Realtime proof; binds the retained Results Night proof catalog. |
| Close-floor dogfood | `npx tsx scripts/dogfood-close-live-floor-command.mts` | Focused local host authorization, concurrency, zero-event, spotlight cleanup, mixed-version and Realtime proof; binds the retained Results Night proof catalog. |
